...
export default async function NewsPage({ params }: UserPageProps) {
  ...
  const moreArticles = allNews
    .filter(article => article.id !== (latestNews?.id || '') && !recentArticles.map(a => a.id).includes(article.id))
    .slice(0, 4);
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2">
            <div className="rounded-lg bg-white p-6 shadow-md">
              ...
              {latestNews.summary && (
                <div className="mb-6">
                  <p className="text-lg font-medium text-gray-800">{latestNews.summary}</p>
                </div>
              )}
                ...
                {recentArticles.map((article, index) => (
                  <div key={article.id} className={`flex gap-4 ${index < recentArticles.length - 1 ? 'border-b border-gray-100 pb-4' : ''}`}>
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-sm text-gray-500">{formatDate(article.createdAt ?? new Date())}</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="mt-12">
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
            {moreArticles.map((article) => (
              <div key={article.id} className="overflow-hidden rounded-lg bg-white shadow-md transition hover:shadow-lg">
